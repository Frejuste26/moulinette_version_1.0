...
class DataValidator:
    ...
    def _normalize_numeric_series(series: pd.Series) -> pd.Series:
        """Normalise une série numérique en tolérant les formats FR (espaces, \xa0, milliers, virgules).

        Règles:
        - Supprime espaces et espaces insécables
        - Si la valeur contient à la fois '.' et ',', on considère '.' comme séparateur de milliers → on supprime '.' puis on remplace ',' par '.'
        - Sinon si elle contient seulement ',' → on remplace ',' par '.'
        ...
        # Stratégie de normalisation milliers/decimales
        has_comma = s.str.contains(',', regex=False, na=False)
        has_dot = s.str.contains('.', regex=False, na=False)

        # Cas mixte: '.' et ',' présents → retirer les points (milliers), convertir virgule en point
        ...
        # Cas virgule seule → virgule devient point
        ...
        # Valeurs vides ou tirets simples restent telles (seront NaN ensuite)
    ...
    def validate_sage_structure(df: pd.DataFrame, required_columns: dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            quantities = DataValidator._normalize_numeric_series(df.iloc[:, qty_col])
            
            if quantities.isna().any():
                invalid_count = quantities.isna().sum()
                # Essayer de fournir un aperçu des 5 premières valeurs invalides pour faciliter le diagnostic
                try:
                    sample_invalid = (
                        df.iloc[:, qty_col]
                          .astype(str)
                    ...
                    sample_msg = f" (exemples: {', '.join(sample_invalid)})"
                except Exception:
                    sample_msg = ""
                return False, f"{invalid_count} valeurs de quantité invalides détectées{sample_msg}"
